...
if ($('.photoUpload-zone').length) {
	...
	$('.photoUpload-zone').each(function () {
		const _this = $(this);
		const photoZoneId = _this.find('.photoUpload-file').attr('id');
		const fileZoneId = _this.find('.photoUpload-file').attr('for');
		const previewId = _this.find('.photoUpload-detail').attr('id');

		const imageInput = _this.find('.photoUpload-file input').attr('id');
		const previewImage = _this.find('.photoUpload-detail').attr('id');
		const cropDimensions = _this.find('.cropDimensions').attr('id');

		const cropButton = _this.find('.cropButton').attr('id');
		const saveButton = _this.find('.saveButton').attr('id');

		const cropWidth = _this.find('.cropWidth').attr('id');
		const cropHeight = _this.find('.cropHeight').attr('id');
		const cropFile = _this.find('.cropFile').attr('id');
	...
	function changeCrop(
		...
		if ($(imageInput).length) {
			...
			$(cropButton).click(function () {
				cutCrop(previewImage, cropButton, saveButton, cropDimensions, cropWidth, cropHeight);
			});

			$(saveButton).click(function () {
				saveCrop(saveButton, previewImage, cropDimensions, cropFile);
			});
		}
	}

	function saveCrop(saveButton, previewImage, cropDimensions, cropFile) {
		...
		cropper.destroy();
		cropper = null;
		document.querySelector(cropDimensions).style.display = 'none';
	}
	...
	function cutCrop(previewImage, cropButton, saveButton, cropDimensions, cropWidth, cropHeight) {
		if (cropper) {
			cropper.destroy();
		}
		...
		cropper = new Cropper(previewImageElem, {
			crop(event) {
				...
				document.querySelector(cropWidth).textContent = Math.round(event.detail.width);
				document.querySelector(cropHeight).textContent = Math.round(event.detail.height);
				document.querySelector(cropDimensions).style.display = 'block';
			}
		});
		document.querySelector(saveButton).style.display = 'block';
