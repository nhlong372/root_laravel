...
class PhotoController
{
    ...
    public function saveStatic($com, $act, $type, Request $request)
    {
        ...
        if (!empty($request->csrf_token)) {
            ...
            $data['type'] = $type;
            $data['com'] = $com;
            if($request->has('options')) $data['options'] = json_encode($request->input('options'));
            ...
            if (!empty($photo)) {
                ...
                if (!empty($itemSave)) {
                    ...
                    if (!empty($this->configType->$type->images)) {
                        $file = $request->file('file');
                        $this->insertImge(PhotoModel::class, $request, $file, $id_insert, 'photo');
                    }
    ...
    public function manAlbum($com, $act, $type, Request $request)
    {
        ...
        if ($keyword) $query->where('namevi', 'like', '%' . $keyword . '%');
        $items =$query->orderBy('numb', 'desc')->orderBy('id', 'desc')->paginate(10);
