    ...
    <div class="container-xxl flex-grow-1 container-p-y container-fluid">
        <div class="app-ecommerce" x-data="$store.changePassword">
            <form class="validation-form" novalidate=""  method="post" action="{{url('user.save')}}" enctype="multipart/form-data">
                <div class="btn-header d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-3 bg-main">
                            ...
                            <div class="card-body card-article" >
                                <div class="row">
                                    ...
                                    <div class="col-12">
                                        ...
                                        <ul class="ps-3 mb-0 ul-requirement">
                                            <li class="mb-1 text-color-dark text-gray-950/60 dark:text-[#b6bee3]" :class="{'text-gray-950/60 dark:text-[#b6bee3]': !$store.changePassword.isValidLength($store.changePassword.changePass.newpassword), 'text-green-500': isValidLength($store.changePassword.changePass.newpassword)}">Dài tối thiểu 8 ký tự - càng nhiều càng tốt</li>
                                            <li class="mb-1 text-color-dark text-gray-950/60 dark:text-[#b6bee3]" :class="{'text-gray-950/60 dark:text-[#b6bee3]': !$store.changePassword.hasSpecialCharacter($store.changePassword.changePass.newpassword), 'text-green-500': $store.changePassword.hasSpecialCharacter($store.changePassword.changePass.newpassword)}">Ít nhất 1 ký tự đặc biệt.</li>
                                            <li class="mb-1 text-color-dark text-gray-950/60 dark:text-[#b6bee3]" :class="{'text-gray-950/60 dark:text-[#b6bee3]': !$store.changePassword.hasLowerAndUpperCase($store.changePassword.changePass.newpassword), 'text-green-500': $store.changePassword.hasLowerAndUpperCase($store.changePassword.changePass.newpassword)}">Ít nhất 1 chữ thường và 1 chữ in hoa.</li>
                                            <li class="text-color-dark text-gray-950/60 dark:text-[#b6bee3]" :class="{'text-gray-950/60 dark:text-[#b6bee3]': !$store.changePassword.doPasswordsMatch, 'text-green-500': $store.changePassword.doPasswordsMatch}">Nhập lại mật khẩu không chính xác.</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="btn-footer d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-3 bg-main">
                    <div class="d-flex align-content-center flex-wrap gap-2">
                        <div class="d-flex gap-2">
                            @if(!empty(request()->query('action')) || empty($item['id']))
                                <button :class="(($store.changePassword.changePass.newpassword.trim() === '' || !$store.changePassword.isValidLength($store.changePassword.changePass.newpassword) || !$store.changePassword.hasSpecialCharacter($store.changePassword.changePass.newpassword) || !$store.changePassword.hasLowerAndUpperCase($store.changePassword.changePass.newpassword) || !doPasswordsMatch))?'disabled':''" type="submit" class="btn btn-primary submit-check"><i class="far fa-save mr-2"></i> Lưu</button>
