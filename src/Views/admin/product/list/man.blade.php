    ...
    <div class="container-xxl flex-grow-1 container-p-y container-fluid">
        <h4>
            <span>Quản lý</span>/<span
                class="text-muted fw-light"></span>{{ $configMan->categories->list->title_main_categories }}
        </h4>
                        ...
                        <tr>
                            <th class="align-middle w-[60px]">
                                <div class="custom-control custom-checkbox my-checkbox">
                                    <input {{ !isPermissions(str_replace('-', '.', $com) . '.' . $type . '.delete') ? 'disabled' : '' }}
                        ...
                        @forelse($items as $k => $v)
                            <tr>
                                ...
                                <td class="align-middle !pl-0">
                                    @component('component.inputNumb', ['numb' => $v['numb'], 'idtbl' => $v['id'], 'table' => 'product_list'])
                                    @endcomponent
                                </td>
                                <td class="align-middle">
                                            ...
                                            @if (!empty($configMan->categories->list->copy_categories))
                                                <div class="dropdown">
                                                    <a id="dropdownCopy" data-url="{{ url('copy') }}"
                                                        data-id="{{ $v['id'] }}" data-table="product_list"
                                ...
                                @if (!empty($configMan->categories->list->show_images_categories))
                                    <td class="align-middle">
                                        <img class="img-preview" onerror="this.src='../assets/images/noimage.png';"
