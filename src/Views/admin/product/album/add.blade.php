                        ...
                        @for ($i = 0; $i < 5; $i++)
                            ...
                            <div class="card">
                                <ul class="nav nav-tabs" id="custom-tabs-three-tab-lang" role="tablist">
                                    ...
                                    <li class="nav-item">
                                        <a class="nav-link {{ $k == 'vi' ? 'active' : '' }}" id="tabs-lang-{{ $i }}" data-bs-toggle="tab" data-bs-target="#tabs-lang-{{ $i }}-{{ $k }}" role="tab" aria-controls="tabs-lang-{{ $k }}" aria-selected="true">{{ $v }}</a>
                                    </li>
                                    @endforeach
                                </ul>
                                <div class="tab-content" id="custom-tabs-three-tabContent-lang">
                                    <div class="form-group last:!mb-0">
                                        <label class="form-label" for="name{{ $i }}">Tiêu đề:</label>
                                        <input type="text" class="form-control for-seo text-sm"
                                            name="dataMultiTemp[{{ $i }}][name]"
                                            id="name{{ $i }}" placeholder="Tiêu đề"
